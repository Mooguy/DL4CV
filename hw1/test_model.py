...
class Softmax(unittest.TestCase):
  ...
  def testIsProbability(self):
    ...
    torch.testing.assert_close(out.sum(dim=1), torch.ones(size=(5,)), msg=f'softmax output rows should sum to 1.\n{debug}')
  ...
  def testNoBatch(self):
    ...
    debug = f'x: {x}\nout: {out}\nref: {ref}'
    torch.testing.assert_close(out, ref, msg=f'softmax output is incorrect (batch_size=1).\n{debug}')
  ...
  def testNumericalStability(self):
    ...
    ref = torch.softmax(x, dim=1)
    debug = f'x: {x}\nout: {out}\nref: {ref}'
    torch.testing.assert_close(out, ref, msg=f'softmax output is not numerically stable.\n{debug}')
...
class CrossEntropy(unittest.TestCase):
  ...
  def testNoBatch(self):
    ...
    debug = f'pred: {pred}\ntarget: {target}\nout: {out}\nref: {ref}'
    torch.testing.assert_close(out, ref, msg=f'cross_entropy output is incorrect (batch_size=1).\n{debug}')
  ...
  def testNumericalStability(self):
    ...
    ref = torch.nn.functional.nll_loss(log_pred, target)
    debug = f'pred: {pred}\ntarget: {target}\nout: {out}\nref: {ref}'
    torch.testing.assert_close(out, ref, msg=f'cross_entropy output is not numerically stable.\n{debug}')
...
class SoftmaxClassifier(unittest.TestCase):
  ...
  def testNoBatch(self):
    ...
    debug = f'x: {x}\nw: {self.w}\nb: {self.b}\nout: {out}\nref: {ref}'
    torch.testing.assert_close(out, ref, msg=f'softmax_classifier output is incorrect (batch_size=1).\n{debug}')
...
class SoftmaxClassifierBackward(unittest.TestCase):
  ...
  def testNoBatch(self):
    ...
    torch.testing.assert_close(self.w.grad, w_grad_ref, msg=f'softmax_classifier_backward w.grad is incorrect (batch_size=1).\n{debug}')
    torch.testing.assert_close(self.b.grad, b_grad_ref, msg=f'softmax_classifier_backward b.grad is incorrect (batch_size=1).\n{debug}')

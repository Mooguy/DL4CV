...
class TestLinear(unittest.TestCase):
  ...
  def testForward(self):
    ...
    ref = F.linear(x, w, b)
    dbg = f'got: {y}\nexpected: {ref}'
    torch.testing.assert_close(y, ref, msg=f'wrong output.\n{dbg}')
  ...
  def testBackward(self):
    ...
    for name, var, ref_grad in to_test:
      dbg = f'got: {var.grad}\nexpected: {ref_grad}'
      torch.testing.assert_close(var.grad, ref_grad, msg=f'wrong gradient of {name}.\n{dbg}')
...
class TestCrossEntropy(unittest.TestCase):
  ...
  def testForward(self):
    ...
    dbg = f'got: {loss}\nexpected: {ref}'
    torch.testing.assert_close(loss, ref, atol=1e-3, rtol=1e-3, msg=f'wrong output:\n{dbg}')
  ...
  def testBackward(self):
    ...
    for name, var, ref_grad in to_test:
      dbg = f'got: {var.grad}\nexpected: {ref_grad}'
      torch.testing.assert_close(var.grad, ref_grad, atol=1e-3, rtol=1e-3, msg=f'wrong gradient of {name}.\n{dbg}')
